...
class ProjectForm(FlaskForm):
    ...
    name = StringField(
        ...
        validators=[
            ...
            Length(min=5, max=100, message="Username must be at least 5 characters."),
        ],
    )
    description = TextAreaField(
        ...
        validators=[
            ...
            Length(
                min=5,
                max=1000,
                message="Description must have at least 5 characters and must not exceed 1000 characters",
            ),
        ],
    )
    start_date = DateField(
        "Start Date",
        format="%Y-%m-%d",
        validators=[DataRequired("Start Date is required")],
    )
    end_date = DateField(
        "End Date", format="%Y-%m-%d", validators=[DataRequired("End Date is required")]
    )
    status = SelectField(
        "Status",
        choices=[
...
class RegistrationForm(FlaskForm):
    ...
    password = PasswordField(
        "Password", validators=[validators.DataRequired(), validate_password]
    )
...
class TaskForm(FlaskForm):
    ...
    title = StringField(
        ...
        validators=[
            ...
            Length(min=5, max=100, message="Title must have at least 5 characters"),
        ],
    )
    description = TextAreaField(
        "Description",
    ...
    assigned_to = SelectField(
        "Assigned To",
        coerce=coerce_to_int_or_none,
        validators=[DataRequired("Assigned To is required")],
    )
    project_id = SelectField(
        "Project",
        coerce=coerce_to_int_or_none,
        validators=[DataRequired("Project is required")],
    )
